...
if __name__ == '__main__':
    ...
    print(f"pure Python:     {time.perf_counter() - t0:.6f} s  (baseline)")
    ...
    try:
        ...
        print(f"CuPy mean:       {statistics.mean(times):.6f} s")
    except ImportError:
        print("CuPy not installed. Run: pip install cupy-cuda12x")

    # --- Numba CUDA ---
    print("\n--- Numba CUDA (GPU, 手写内核) ---")
    bench_numba_cuda()  # 预热 + 编译
    times = []
    for _ in range(10):
        t0 = time.perf_counter()
        bench_numba_cuda()
        times.append(time.perf_counter() - t0)
    print(f"Numba CUDA mean: {statistics.mean(times):.6f} s")
    ...
    ║  方案选择指南                                            ║
    ╠═══════════════════════════════════════════════════════════╣
    ║  计算是 NumPy 能表达的 → CuPy （改一行 import）          ║
    ║  计算有分支/逻辑但元素独立 → Numba @cuda.jit（Python 写内核）║
    ║  需要终极性能 / 复杂数据流 → CUDA C++（最灵活也最累）    ║
    ╚═══════════════════════════════════════════════════════════╝
    """)
